  ...
  def writeFileHeader(height, stride) do
    ...
    fileHeader =
      ['B'] ++
        ...
        [<<fileSize >>> 24>>] ++
        List.duplicate(<<0>>, 4) ++
        [<<Bmpgen.fileHeaderSize() + Bmpgen.infoHeaderSize()>>] ++ List.duplicate(<<0>>, 3)
...
defmodule Main do
  ...
  def main do
    ...
    PolyHok.spawn_jit(
      &RayTracer.raytracing/4,
      {trunc(width / 16), trunc(height / 16), 1},
      {16, 16, 1},
      [width, height, refSphere, refImag]
    )
